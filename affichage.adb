package body affichage is
    ...
    begin
        case pos is
            ...
            when HAUT => put("  "); for i in Colonnes'range loop put("  "); put(i,0); put(" "); end loop; new_line;
                         put("  ╔"); for i in 1..35 loop put("═"); end loop; put("╗");
            when BAS => put("  ╚"); for i in 1..35 loop put("═"); end loop; put("╝");
    ...
    begin
        put("  ║"); for i in 1..Colonnes'last-1 loop put("───┼"); end loop; put("───║"); new_line;  
    end afficherLigneHorizontale;
    
    function caractereCaseJeu(lig : Lignes; col : Colonnes; jeu : Damier) return String is
    mcase : CaseJeu := jeu(lig,col);
    begin
        case mcase.mtype is
            when VIDE => return " ";
            when OCCUPE => if(mcase.mpion.camp = BLANC) then return "☺"; else return "☻"; end if;
        end case;
   ...
   procedure afficherAideSaisieCoups is
    begin
        ...
        new_line; new_line;
        put("On saisie les coordonnées sous la forme 'XY' où X designe la ligne (lettre), et Y la colonne (nombre) dans le damier.");
        new_line;
        put("La première coordonnée attendue est celle du pion qui exécute le coup, et la/les coordonnées suivantes (séparées par un espace) sont les cases où celui-ci doit jouer.");
        new_line;
        put("Ex : B4 B5 D5 désigne le mouvement du pion en B4 vers la case B5 puis D5.");
        new_line;
        put("Pour plus d'informations sur les mouvements se référer aux règles du Kick-Or-Flip.");
        new_line; new_line;
